...
* An **event** is a subset of the sample space, that is, a collection of (some, all, or none of the) outcomes of the random phenomenon. For our puposes all subsets of the sample space are events, however, in advanced books some subsets are excluded.
...
* The null set (written $\emptyset$) is the set containing no elements (in this case, the event containing no outcomes).  In any probability model, $\emptyset$ and $S$ are events (that is always, even in advanced books that exclude some subsets from being events).
...
NOTE: Rules 1-3 are what you will see, in advanced books, labeled as axioms.  These are all you need for the statement just mentioned above to make sense.  Rule 4 is a theorem that can be derived from Rules 1-3.  Rule 5 is essentially the definition of independence given in advanced books.
...
* Example Problem 1: A four-sided die is thrown.  Each side has a different color and the color of the side facing down is observed.  The colors are: red, green, blue, and white.  What is the sample space of this random phenomenon and give an example of a random variable on this sample space.

* Example Problem 2: A random student is chosen from this class.  What is the sample space?  What is an example of a random variable on this sample space.  Hint: you might want to ask this student a question to determine the random variable.

* Example Problem 3: Three random students are chosen from this class.  What is the sample space.  What is an example of a random variable on this sample space.  Same hint: ask the same question to all three students.
...
* There will be one and only one such ordered pair for each element of the sample space.

* Thus to define a random variable, there needs to be a number given to _every_ outcome.   The following is an example of a random variable on the sample space of Example Problem 1:

$$\begin{eqnarray} \mbox{red} & \mapsto & 10 \\ \mbox{green} & \mapsto & 15 \\ \mbox{blue} & \mapsto & -5 \\ \mbox{white} & \mapsto & -10 \end{eqnarray} $$
These might be interpreted as the winning of a game of chance played with the four-sided die.  We say red maps to 10, green maps to 15, blue maps to -5, and white maps to -10.
...
The repeated 15 is OK.  What is shown above is also a correct answer to Example Problem 1.

* However the same outcome **CANNOT* have two numbers.  The follow is **NOT** and example of a random variable:
...
The repeated red is **NOT OK**.

* In precalculus you might have seen functions of the form $y = 2 x + 1$ or $y = x^2$.  These are functions that map real numbers to real numbers.  There are infinitely many real numbers, so you will never be able to list all ordered pairs in the functions.  But we can list a few of them.  Or you can just write the formulas as

$$ x \mapsto 2 x + 1 $$

Here $x$ takes the role of "outcome", and $2 x + 1$ is the number that $x$ maps to.  If the sample space were all real numbers, this function would be a random variable on that sample space.  To find the number that the outcome maps to, just apply the formula.  E.g.: since $2 \times 0 + 1 = 1$, it is true that $0 \mapsto 1$

In the second function, 

$$x \mapsto x^2$$

For example, if we plug in 2 for $x$ we get $2^2=4$, so $2 \mapsto 4$.  We could plug in any number for $x$ to find the number that $x$ maps to.

* In general, if $y = f(x)$ then $x \mapsto y$.

* The stipulation that the same outcome doesn't get two numbers (e.g. red is not repeated) is a equivalent to the vertical line test that you might be familiar with from precalculus.
